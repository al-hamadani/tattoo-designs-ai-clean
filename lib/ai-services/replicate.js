...
/**
 * Replicate AI Service (JavaScript version)
 * Clean, robust abstraction for text-to-image and inpainting (gap filler)
 */
export class ReplicateService extends BaseAIService {
  ...
  constructor(apiToken = process.env.REPLICATE_API_TOKEN) {
    ...
    /**
     * @type {string} - The exact model version string from replicate.com
     */
    this.modelVersion = 'lucataco/sdxl-inpainting:a5b13068cc81a89a4fbeefeccc774869fcb34df4dbc92c1555e0f2771d49dde7'; // <-- Set your model version here
  ...
  async uploadToReplicate(bufferOrB64, filename) {
    // Debug: log Replicate SDK version
    console.log('[debug] uploading file; sdk version', (await import('replicate/package.json')).version);
    const buf = Buffer.isBuffer(bufferOrB64) ? bufferOrB64 : this.base64ToBuffer(bufferOrB64);
