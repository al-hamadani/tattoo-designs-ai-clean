 ...
 * Truncates enhanced prompts to specified token limit, prioritizing importance
 */
export function truncateToTokenLimit(prompt, maxTokens = 77) {
  ...
  // Split prompt into components by importance
  const parts = prompt.split(', ');
  
  // Priority order: core description, style, technique, then others
  const priorityOrder = [
    // Core elements (highest priority)
    parts.find(p => p.includes('tattoo') || p.includes('design')),
    parts.find(p => p.includes('style') || p.includes('traditional') || p.includes('realism') || p.includes('geometric')),
    parts.find(p => p.includes('technique') || p.includes('linework') || p.includes('shading')),
    
    // Secondary elements
    parts.find(p => p.includes('complexity') || p.includes('simple') || p.includes('complex')),
    parts.find(p => p.includes('size') || p.includes('small') || p.includes('large')),
    parts.find(p => p.includes('composition') || p.includes('flow') || p.includes('depth')),
    
    // Quality and background elements
    parts.find(p => p.includes('professional') || p.includes('quality')),
    parts.find(p => p.includes('background') || p.includes('white')),
    parts.find(p => p.includes('craftsmanship') || p.includes('artist'))
  ].filter(Boolean);
  ...
  for (const part of priorityOrder) {
    ...
    if (currentTokens + partTokens <= maxTokens) {
      truncatedPrompt += (truncatedPrompt ? ', ' : '') + part;
  ...
  return truncatedPrompt || prompt.substring(0, 200); // Fallback if truncation fails
}