...
try {
  ...
  // console.warn('Canvas package not available, using fallback mask processing');
  createCanvas = null;
  loadImage = null;
}
...
export class MaskProcessor {
  ...
  async initialize() {
    // If canvas is not available, return false to use fallback
  ...
  getCoveragePercentage() {
    if (!this.ctx) {
      return 15; // Default fallback coverage
    }
  ...
  detectShape() {
    if (!this.ctx) {
      return 'irregular'; // Default fallback shape
    }
  ...
  getSizeRecommendation() {
    if (!this.ctx) {
      return 'medium'; // Default fallback size
    }
  ...
  async analyzeMask() {
    ...
    // If canvas is not available, provide basic fallback analysis
    if (!initialized) {
      return {
        hasData: true,
        boundingBox: { x: 0, y: 0, width: 100, height: 100, centerX: 50, centerY: 50 },
        coveragePercentage: '15.0',
        estimatedCoverage: 'medium',
        suggestedSize: 'medium',
        shapeHint: 'irregular',
        aspectRatio: '1.00',
        promptHints: ['organic flow', 'adaptive shape'],
        fallback: true
      };
    }
